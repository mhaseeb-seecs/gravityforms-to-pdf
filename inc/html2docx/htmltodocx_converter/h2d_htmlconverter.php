...
function htmltodocx_html_allowed_children($tag = null)
{
    ...
    $allowed_children = array(
        ...
        // p does not nest - simple_html_dom will create a flat set of paragraphs if it finds nested ones.
        'div' => array(
            'a',
            'em',
            'i',
        ...
        'a' => array('text'),
        // PHPWord doesn't allow elements to be placed in link elements
        'em' => array('a', 'strong', 'b', 'br', 'span', 'code', 'u', 'sup', 'text'),
        // Italic
        'i' => array('a', 'strong', 'b', 'br', 'span', 'code', 'u', 'sup', 'text'),
        // Italic
        'strong' => array('a', 'em', 'i', 'br', 'span', 'code', 'u', 'sup', 'text'),
        // Bold
        'b' => array('a', 'em', 'i', 'br', 'span', 'code', 'u', 'sup', 'text'),
        // Bold
        'sup' => array('a', 'em', 'i', 'br', 'span', 'code', 'u', 'text'),
        // Superscript
        'u' => array('a', 'em', 'strong', 'b', 'i', 'br', 'span', 'code', 'sup', 'text'),
        // Underline - deprecated - but could be encountered.
        'ul' => array('li'),
        'ol' => array('li'),
        ...
        // PHPWord does not allow you to insert a table into a table cell
        'th' => array(
            'p',
            'a',
            'em',
        ...
        'br' => array(),
        'code' => array(),
        // Note, elements nested inside the code element do not work! (Perhaps simpleHTMLDom isn't recognising them).
        'span' => array('a', 'em', 'i', 'strong', 'b', 'img', 'br', 'span', 'code', 'sup', 'text'),
        // Used for styles - underline
        'text' => array(),
        // The tag name used for elements containing just text in SimpleHtmlDom.
    );
...
function htmltodocx_insert_html(&$phpword_element, $html_dom_array, &$state = array())
{
    ...
    // Tables:
    if (in_array('td', $state['parents']) || in_array('th',
            $state['parents']) || (isset($state['table_allowed']) && !$state['table_allowed'])) {
...
//        echo PHP_EOL;
        switch ($element->tag) {
            ...
            case 'h6':
                ...
                if ($state['structure_document'] && in_array($element->tag,
                        array('h1', 'h2', 'h3', 'h4', 'h5', 'h6')) && is_object($state['phpword_object'])) {
                    ...
                    $state['phpword_object']->addTitleStyle($state['structure_headings'][$element->tag],
                        $state['current_style']);
                    $phpword_element->addTitle(htmltodocx_clean_text($element->innertext),
                        $state['structure_headings'][$element->tag]);
                    break;
                }
            ...
            case 'table':
                if (in_array('table', $allowed_children)) {
                    ...
                    if (!$state['table_allowed'] || in_array('td', $state['parents']) || in_array('th',
                            $state['parents'])) {
                        ...
                        if (is_object($state['phpword_object']) && method_exists($state['phpword_object'],
                                'addTableStyle')) {
            ...
            case 'a':
                ...
                if ($state['context'] == 'section') {
                    ...
                    // document:
                    $state['textrun']->addLink(htmltodocx_url_encode_chars($href),
                        htmltodocx_clean_text($element->innertext), $state['current_style']);
                } else {
